...
const QDrant = {
  ...
  similarityResponse: async function ({
    ...
    const responses = await client.search(namespace, searchParams);
  ...
  addDocumentToNamespace: async function (
    ...
    try {
      ...
      // Use parallel embedder if multiple API keys are available
      const hasMultipleKeys = process.env.GEMINI_API_KEY_1 && process.env.GEMINI_API_KEY_2;
      let vectorValues;
      let textChunks;
      
      if (hasMultipleKeys && process.env.EMBEDDING_ENGINE === 'gemini') {
        console.log("[Qdrant] Using parallel Gemini embedder with multiple API keys");
        const { ParallelGeminiEmbedder } = require("../../EmbeddingEngines/gemini/ParallelGeminiEmbedder");
        const parallelEmbedder = new ParallelGeminiEmbedder();
        
        const chunkSize = TextSplitter.determineMaxChunkSize(
          await SystemSettings.getValueOrFallback({
            label: "text_splitter_chunk_size",
          }),
          8000  // Use reasonable max for Gemini
        );
        ...
        const textSplitter = new TextSplitter({
          chunkSize: chunkSize,
          chunkOverlap: Math.min(chunkOverlap, Math.floor(chunkSize / 2)), // Ensure overlap < chunkSize
        ...
        vectorValues = await parallelEmbedder.embedChunks(textChunks);
      } else {
        ...
        console.log("Snippets created from document:", textChunks.length);
        vectorValues = await EmbedderEngine.embedChunks(textChunks);
      }
  ...
  performSimilaritySearch: async function ({
    ...
    if (metadataFilters && Object.keys(metadataFilters).length > 0) {
      qdrantFilter = this.buildQdrantFilter(metadataFilters);
      console.log(`[Qdrant] Applying metadata filters:`, metadataFilters);
    }
   ...
   */
  buildQdrantFilter: function (metadataFilters) {
    const must = [];

    // Add filters for each metadata field
    if (metadataFilters.court) {
      must.push({
        key: "court",
        match: { value: metadataFilters.court }
      });
    }
    ...
    if (metadataFilters.year) {
      must.push({
        key: "year",
        match: { value: metadataFilters.year }
      });
    }

    if (metadataFilters.jurisdiction) {
      must.push({
        key: "jurisdiction",
        match: { value: metadataFilters.jurisdiction }
      });
    }

    if (metadataFilters.case_type) {
      must.push({
        key: "case_type",
        match: { value: metadataFilters.case_type }
      });
    }

    if (metadataFilters.judge) {
      must.push({
        key: "judge",
        match: { value: metadataFilters.judge }
      });
    }

    // Return filter object if we have conditions
