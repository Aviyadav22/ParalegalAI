...
class HybridReranker {
  ...
  static async rerank(vectorResults = [], metadataResults = [], query = "", options = {}) {
    const {
      semanticWeight = 0.4,          // Weight for semantic similarity (Qdrant)
      rerankerWeight = 0.3,          // Weight for hosted reranker model
      bm25Weight = 0.2,              // Weight for BM25 keyword matching
      ...
      bm25Results = [],              // BM25 search results
    } = options;
    ...
    try {
      ...
      // Step 2.5: Process BM25 search results
      for (const result of bm25Results) {
        ...
        if (documentMap.has(docId)) {
          // Document found in both vector and BM25 - boost it
          const existing = documentMap.get(docId);
          existing.bm25Score = bm25Score;
          existing.source = existing.source === 'vector' ? 'hybrid_vector_bm25' : existing.source;
        } else {
          // Document only from BM25 search
          documentMap.set(docId, {
            ...result,
            docId,
            vectorScore: 0,
      ...
      // Step 5: Calculate combined scores with enhanced weighted formula
      // final_score = (0.4 * semantic) + (0.3 * reranker) + (0.2 * bm25) + (0.1 * metadata)
      const scoredResults = [];
      for (const [docId, doc] of documentMap) {
        // Normalize scores to 0-1 range
        const semanticScore = Math.min(Math.max(doc.vectorScore, 0), 1);
        const bm25ScoreNorm = Math.min(Math.max(doc.bm25Score, 0), 1);
        const metadataScore = Math.min(Math.max(doc.metadataScore, 0), 1);
        const hostedRerankerScore = rerankerScores.get(docId) || 0.5;

        // Weighted combination using enhanced legal RAG prioritization
        let combinedScore = 
          (semanticScore * semanticWeight) +
          (hostedRerankerScore * rerankerWeight) +
          (bm25ScoreNorm * bm25Weight) +
          (metadataScore * metadataWeight);

        // Boost for documents found in multiple sources (high confidence)
        const sourceCount = [
          doc.vectorScore > 0,
          doc.bm25Score > 0,
          doc.metadataScore > 0
        ...
        if (sourceCount >= 2) {
          combinedScore *= 1.1; // 10% boost for multi-source matches
        }
      ...
      console.log(`[HybridReranker] Sources: ${vectorResults.length} vector, ${bm25Results.length} BM25, ${metadataResults.length} metadata`);
      console.log(`[HybridReranker] Scoring: ${(semanticWeight*100).toFixed(0)}% semantic + ${(rerankerWeight*100).toFixed(0)}% reranker + ${(bm25Weight*100).toFixed(0)}% BM25 + ${(metadataWeight*100).toFixed(0)}% metadata`);
  ...
  static explainScores(results, topN = 3) {
    ...
    console.log(`Formula: (0.4 × semantic) + (0.3 × reranker) + (0.2 × BM25) + (0.1 × metadata)`);
