...
const { verifyFileAccessMiddleware, generateFileAccessToken } = require("../../utils/originalFiles/secureAccess");
...
function apiOriginalFilesEndpoints(app) {
   ...
   */
  app.get("/original-files/:fileId/token", async (request, response) => {
    try {
      const { fileId } = request.params;
      const { expiresIn = 300 } = request.query; // Default 5 minutes
      ...
      // Generate token
      const token = generateFileAccessToken(fileId, { 
      ...
      response.status(200).json({
        ...
        token,
        expiresIn: parseInt(expiresIn),
        url: `/api/original-files/${fileId}?token=${token}`
      });
    } catch (error) {
      console.error("Error generating file access token:", error);
   ...
   */
  app.get("/original-files/:fileId", [verifyFileAccessMiddleware], async (request, response) => {
    try {
      const { fileId } = request.params;
