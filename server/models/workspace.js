...
const Workspace = {
  ...
  getWithUser: async function (user = null, clause = {}, includeDocuments = false) {
    ...
    try {
      ...
      return {
        ...workspace,
        // Only include documents array when explicitly requested to avoid overhead
        // with workspaces that have large document counts (e.g., 50K+ documents)
        ...(includeDocuments && { documents: workspace.documents || [] }),
        ...(!includeDocuments && { documents: [] }),
        contextWindow: this._getContextWindow(workspace),
        // Skip expensive token count query for performance
  ...
  get: async function (clause = {}, includeDocuments = false) {
    try {
      const workspace = await prisma.workspaces.findFirst({
        where: clause,
        ...(includeDocuments && {
      ...
      return {
        ...
        contextWindow: this._getContextWindow(workspace),
        // Skip expensive token count query for performance
        // This is only needed in specific contexts, not on every workspace load
        currentContextTokenCount: 0,
      };
    } catch (error) {
      console.error(error.message);
