...
const Document = {
  ...
  addDocuments: async function (workspace, additions = [], userId = null) {
    ...
    // Use parallel processing for large batches (10+ documents)
    if (additions.length >= 10) {
      ...
      const processor = new ParallelDocumentProcessor({
        concurrency: parseInt(process.env.PARALLEL_CONCURRENCY) || 8,
        batchSize: parseInt(process.env.PARALLEL_BATCH_SIZE) || 100,
        dbBatchSize: parseInt(process.env.PARALLEL_DB_BATCH_SIZE) || 50,
    ...
    // Sequential processing for small batches (< 10 documents)
    ...
    for (const path of additions) {
      ...
      try {
        const createdDoc = await prisma.workspace_documents.create({ data: newDoc });
        embedded.push(path);

        // DISABLED: Legal metadata extraction (Stage 2 feature)
        // Keeping only vector search for now
      } catch (error) {
        console.error(error.message);
      }
    }
