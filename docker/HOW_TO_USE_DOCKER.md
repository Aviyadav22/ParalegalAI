# How to use Dockerized Paralegal AI

Use the Dockerized version of ParalegalAI for a much faster and complete startup of ParalegalAI.

### Minimum Requirements

> [!TIP]
> Running ParalegalAI on AWS/GCP/Azure?
...
\*ParalegalAI by default uses a built-in vector database powered by [LanceDB](https://github.com/lancedb/lancedb)

\*ParalegalAI by default embeds text on instance privately [Learn More](../server/storage/models/README.md)

## Recommend way to run dockerized ParalegalAI!
...
> the docker container using ParalegalAI as `localhost:xxxx` will not resolve for the host system.
>
...
```shell
docker pull mintplexlabs/paralegalai
```

<table>
<tr>
<th colspan="2">Mount the storage locally and run ParalegalAI in Docker</th>
</tr>
<tr>
<td>
  Linux/MacOs
...
```shell
export STORAGE_LOCATION=$HOME/paralegalai && \
...
$env:STORAGE_LOCATION="$HOME\Documents\paralegalai"; `
...
services:
  paralegalai:
    image: mintplexlabs/paralegalai
    container_name: paralegalai
    ports:
    - "3001:3001"
    cap_add:
      - SYS_ADMIN
    ...
    volumes:
      - paralegalai_storage:/app/server/storage
    restart: always
...
Go to `http://localhost:3001` and you are now using ParalegalAI! All your data and progress will persist between
...
- `git clone` this repo and `cd paralegal-ai` to get to the root directory.
- `touch server/storage/paralegalai.db` to create empty SQLite DB file.
- `cd docker/`
...
The integrations below are templates or tooling built by the community to make running the docker experience of ParalegalAI easier.

### Use the Midori AI Subsystem to Manage ParalegalAI

Follow the setup found on [Midori AI Subsystem Site](https://io.midori-ai.xyz/subsystem/manager/) for your host OS
After setting that up install the ParalegalAI docker backend to the Midori AI Subsystem.
...
Then in docker you need to replace that localhost part with `host.docker.internal`. For example, if running Ollama on the host machine, bound to http://127.0.0.1:11434 you should put `http://host.docker.internal:11434` into the connection URL in ParalegalAI.
