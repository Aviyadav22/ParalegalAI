...
export default function AccountModal({ user, hideModal }) {
  ...
  // Load subscription info and usage
  useEffect(() => {
    (async () => {
      try {
        const current = await System.subscription.current();
        const usageRes = await System.subscription.usage();
        setPlan(current?.subscription || null);
        setUsage(usageRes?.usage || null);
      } catch (e) {
        // ignore if backend not present
      } finally {
  ...
  const handleUpgrade = async () => {
    setUpgrading(true);
    try {
      const res = await System.subscription.update("premium");
      if (res?.success) {
        showToast("Upgraded to Premium.", "success", { clear: true });
        // Refresh plan
        const current = await System.subscription.current();
        setPlan(current?.subscription || null);
      } else {
        showToast(res?.error || "Upgrade failed.", "error", { clear: true });
      }
    } catch (e) {
      showToast("Upgrade service not available.", "error", { clear: true });
    } finally {
      setUpgrading(false);
    }
  };
        ...
        >
          <form onSubmit={handleUpdate} className="space-y-6">
            ...
            <div className="flex flex-col gap-y-4 px-6">
              ...
              {/* Usage & Plan */}
              <div className="mt-6 p-4 rounded-lg border border-theme-modal-border">
                <h4 className="text-white font-semibold mb-2">Usage & Plan</h4>
                {loadingPlan ? (
                  <p className="text-white/70 text-sm">Loading...</p>
                ) : (
                  <div className="text-white/90 text-sm space-y-1">
                    <p>
                      Plan: <b>{plan?.tier || user.subscriptionTier || "free"}</b> ({plan?.status || user.subscriptionStatus || "active"})
                    </p>
                    {usage ? (
                      <>
                        <p>Messages today: <b>{usage.messagesToday ?? 0}</b>{usage.dailyLimit ? ` / ${usage.dailyLimit}` : ""}</p>
                        {usage.remainingToday != null && (
                          <p>Remaining today: <b>{usage.remainingToday}</b></p>
                        )}
                      </>
                    ) : (
                      <p className="text-white/70">Usage data unavailable.</p>
                    )}
                    {(plan?.tier || user.subscriptionTier) !== "premium" && (
                      <div className="pt-2">
                        <button
                          type="button"
                          onClick={handleUpgrade}
                          disabled={upgrading}
                          className="transition-all duration-300 bg-white text-black hover:opacity-60 px-3 py-1.5 rounded-md text-sm"
                        >
