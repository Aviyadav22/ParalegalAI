...
export default function UploadFile({
  ...
  return (
    <div>
      <div
        className={`w-[560px] border-dashed border-[2px] border-theme-modal-border border-[#686C6F] rounded-2xl bg-theme-bg-primary transition-colors duration-300 p-3 ${
          ready
            ? " bg-[#E0F2FE] cursor-pointer hover:bg-theme-bg-secondary hover:bg-transparent"
            : "cursor-not-allowed"
        }`}
        {...getRootProps()}
      >
        ...
        {ready === false ? (
          <div className="flex flex-col items-center justify-center h-full">
            <CloudArrowUp className="w-8 h-8 text-white/80 invert" />
      ...
      <form onSubmit={handleSendLink} className="flex gap-x-2">
        ...
        <button
          disabled={fetchingUrl}
          type="submit"
          className="disabled:bg-white/20 disabled:text-slate-300 disabled:border-slate-400 disabled:cursor-wait bg bg-transparent hover:bg-slate-200 hover:text-slate-800 w-auto border border-white border-theme-modal-border text-sm text-white p-2.5 rounded-lg"
        >
          {fetchingUrl
