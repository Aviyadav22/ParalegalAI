...
function FileUploadProgressComponent({
  ...
  if (rejected) {
    return (
      <div
        ...
        } h-14 px-2 py-2 flex items-center gap-x-4 rounded-lg bg-error/40 bg-error/30 border-solid border-error/40 border border-transparent`}
      >
        ...
        <div className="flex flex-col">
          <p className="text-white text-red-600 text-xs font-semibold">
            {truncate(file.name, 30)}
          </p>
          <p className="text-red-100 text-red-600 text-xs font-medium">
  ...
  return (
    <div
      className={`${
        isFadingOut ? "file-upload-fadeout" : "file-upload"
      } h-14 px-2 py-2 flex items-center gap-x-4 rounded-lg bg-zinc-800 border-solid border-theme-modal-border bg-theme-bg-sidebar border border-white/20 shadow-md`}
    >
      <div className="w-6 h-6 flex-shrink-0">
        {status !== "complete" ? (
      ...
      <div className="flex flex-col">
        <p className="text-white text-theme-text-primary text-xs font-medium">
          {truncate(file.name, 30)}
        </p>
        <p className="text-white/80 text-theme-text-secondary text-xs font-medium">
