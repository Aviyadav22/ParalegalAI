...
export default function DocumentSettings({ workspace, systemSettings }) {
  ...
  async function fetchKeys(loadDocuments = true) {
    setLoading(true);
    const localFiles = await System.localFiles();
    
    // Fetch workspace documents only when needed
    let documentsInWorkspace = [];
    if (loadDocuments) {
      const currentWorkspace = await Workspace.bySlug(workspace.slug, true);
      documentsInWorkspace = currentWorkspace?.documents?.map((doc) => doc.docpath) || [];
    }
  ...
  useEffect(() => {
    // Load documents when component mounts (only when Documents tab is active)
    fetchKeys(true);
  }, []);
