      ...
      <div className="px-8 pb-8" onContextMenu={handleContextMenu}>
        <div className="flex flex-col gap-y-6">
          <div className="flex items-center justify-between w-[560px] px-5 relative">
            ...
            <div className="relative">
              <input
                ...
                className="border-none search-input bg-theme-settings-input-bg text-white placeholder:text-theme-settings-input-placeholder focus:outline-primary-button active:outline-primary-button outline-none text-sm rounded-lg pl-9 pr-2.5 py-2 w-[250px] h-[32px] border-theme-modal-border border"
              />
              <MagnifyingGlass
                size={14}
            ...
            >
              <Plus
                size={18}
                weight="bold"
                className="text-theme-text-primary text-[#0ba5ec]"
              />
              <div className="text-theme-text-primary text-[#0ba5ec] text-xs font-bold leading-[18px]">
          ...
          <div className="relative w-[560px] h-[310px] bg-theme-settings-input-bg rounded-2xl overflow-hidden border border-theme-modal-border">
            <div className="absolute top-0 left-0 right-0 z-10 rounded-t-2xl text-theme-text-primary text-xs grid grid-cols-12 py-2 px-8 border-b border-white/20 border-theme-modal-border bg-theme-settings-input-bg">
            ...
            {amountSelected !== 0 && (
              <div className="absolute bottom-[12px] left-0 right-0 flex justify-center pointer-events-none">
                <div className="mx-auto bg-white/40 bg-white rounded-lg py-1 px-2 pointer-events-auto shadow-lg">
                  <div className="flex flex-row items-center gap-x-2">
                    <button
                      onClick={moveToWorkspace}
                      onMouseEnter={() => setHighlightWorkspace(true)}
                      onMouseLeave={() => setHighlightWorkspace(false)}
                      className="border-none text-sm font-semibold bg-white bg-[#E0F2FE] h-[30px] px-2.5 rounded-lg hover:bg-neutral-800/80 hover:text-white text-[#026AA2] hover:bg-[#026AA2] hover:text-white"
                    >
                    ...
                    <div className="relative">
                      <button
                        ...
                        className="border-none text-sm font-semibold bg-white bg-[#E0F2FE] h-[32px] w-[32px] rounded-lg text-dark-text hover:bg-neutral-800/80 hover:text-white text-[#026AA2] hover:bg-[#026AA2] hover:text-white flex justify-center items-center group"
                      >
                        <MoveToFolderIcon className="text-dark-text text-[#026AA2] group-hover:text-white" />
                      </button>
                      {showFolderSelection && (
                    ...
                    <button
                      onClick={deleteFiles}
                      className="border-none text-sm font-semibold bg-white bg-[#E0F2FE] h-[32px] w-[32px] rounded-lg text-dark-text hover:bg-neutral-800/80 hover:text-white text-[#026AA2] hover:bg-[#026AA2] hover:text-white flex justify-center items-center"
                    >
...
function DirectoryTooltips() {
  return (
    <Tooltip
      ...
      place="bottom"
      delayShow={800}
      className="tooltip invert invert-0 z-99 max-w-[200px]"
      render={({ content }) => {
        const data = safeJsonParse(content, null);
        if (!data) return null;
        return (
          <div className="text-xs">
            <p className="text-white invert font-medium break-all">
              {data.title}
            </p>
