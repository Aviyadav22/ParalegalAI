...
function ItemDirection({ active, reverse, onSelect, msg }) {
  return (
    <button
      ...
      type="button"
      className={`flex:1 p-4 bg-transparent hover:bg-gray-100 rounded-xl border w-[250px] ${active ? "border-primary-button" : " border-theme-sidebar-border"}`}
      onClick={onSelect}
    >
      <div className="space-y-4">
        {Array.from({ length: 3 }).map((_, index) => (
          ...
          >
            <div
              className={`w-4 h-4 rounded-full ${index % 2 === 0 ? "bg-primary-button" : "bg-black"} flex-shrink-0`}
            />
            <div className="bg-gray-200 rounded-2xl px-4 py-2 h-[20px] w-full" />
          </div>
        ))}
      </div>
    </button>
