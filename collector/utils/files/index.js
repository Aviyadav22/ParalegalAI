...
/**
 * Establish a base storage directory for collector operations.
 * In production, if STORAGE_DIR is not provided or .env not loaded, fall back to server/storage.
 */
const baseStorage =
...
/**
 * The folder where documents are stored when processed by the collector.
 */
const documentsFolder = path.resolve(baseStorage, `documents`);

/**
 * The folder where direct uploads are stored when processed by the collector.
 * These are files that were DnD'd into UI and are not embedded/selectable from the file picker.
 */
const directUploadsFolder = path.resolve(baseStorage, `direct-uploads`);
